...
function Home() {
  ...
  const handleSearch = async () => {
    try {
      ...
      const response = await API.searchRecipes(search, token);
  ...
  const confirmDelete = async () => {
    try {
      ...
      await API.deleteRecipe(deleteId, token);

      setRecipes(recipes.filter((item) => item._id !== deleteId));
  ...
  return (
    <>
      ...
      <div className="home">
        <h1 className="title">Find Your Favorite Recipe 🍳</h1>
        ...
        <div className="recipe-grid">
          ...
          <RecipeCard showAddButton={true} onAdd={() => setShowModal(true)} />
